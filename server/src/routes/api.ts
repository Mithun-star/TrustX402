...
import { getAllServices, getServiceById, seedServiceRegistry, registerNewService } from '../services/registry/ServiceRegistry.js';
...
apiRouter.post('/services', async (req: Request, res: Response) => {
  try {
    const service = await registerNewService(req.body);
...
apiRouter.post('/agent/run', async (req: Request, res: Response) => {
  try {
    const { userRequest, targetServiceId, conversationHistory } = req.body;
    if (!userRequest) {
      return res.status(400).json({ success: false, error: 'userRequest parameter is required.' });
    }
    const agentRun = await runAgentWorkflow(userRequest, targetServiceId, conversationHistory);
